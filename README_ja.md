...
Mado は `MADO_NVIM`、`PATH`、および macOS / Windows の一般的なインストール先から Neovim を探します。

## リリース版の導入

最初の GitHub リリースでは、macOS Apple Silicon 向けビルドをダウンロードして `Mado.app` を開いてください。
初回起動時に Gatekeeper の警告が出た場合は、ダウンロード元を確認したうえで Finder の **Open** を使うか、
quarantine を解除してください。

## 設定の考え方

Neovim の配色、ハイライト、カーソル形状は Neovim 側が管理します。Mado が担当するのはフォントや初期ウィンドウサイズ
のようなネイティブウィンドウまわりです。たとえば `init.lua` には次のように書けます。
...
- `window.blur`
  - `true` / `false` のブール値です。
  - `true` にすると、対応しているプラットフォームでは透明部分に OS のブラー効果を依頼します。
  - 既定値は `false` です。
...
テストには grid、input、IME、RPC に加えて、Neovim がインストールされていれば実際の `nvim --clean --embed`
接続テストも含まれます。

## macOS アプリとファイル関連付け

PowerShell から ad-hoc 署名付きのアプリバンドルをビルドできます。
...
**Open With → Mado** を選ぶか、**Get Info → Open with → Change All** で既定アプリに設定できます。
...
WiX v4 向けのインストーラー定義は `packaging/windows/Mado.wxs` にあります。
